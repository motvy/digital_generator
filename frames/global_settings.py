...
class GlobalSettings():
    ...
    def change_frequency(self):
        ...
        if not self.frequency_input.text() or int(self.frequency_input.text()) != self.db_settings_dict['frequency']:
    ...
    def apply_settings(self):
        ...
        if not frequency or int(frequency) < config.DEFAULT_GLOBAL_SETTINGS['frequency']['min']:
            frequency = config.DEFAULT_GLOBAL_SETTINGS['frequency']['min']
        elif int(frequency) > config.DEFAULT_GLOBAL_SETTINGS['frequency']['max']:
            frequency = config.DEFAULT_GLOBAL_SETTINGS['frequency']['max']
        else:
    ...
    def hasUnsaved(self):
        for label in (self.frequency_source_label, self.frequency_label, self.length_label, self.amplitude_label):
            if '*' in label.text():
                return True
        return False
